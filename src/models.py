...
class LogisticRegression(nn.Module):
    def __init__(self,
        ...
        self.fc = nn.Linear(in_features=n_in_features, out_features=n_classes)
        self.softmax = nn.LogSoftmax(dim=1)

    def forward(self, x):
        fw_output = self.fc(x)
        fw_output = self.softmax(fw_output)
        return fw_output
    

# 2 ) CNN
# based on Kim (2014) as well as content from the machine learning course 
# (https://github.com/fredrikwahlberg/5LN708/blob/master/Lecture%2007%20-%20CNN%20for%20text%20classification.ipynb)
class CNN(nn.Module):
    def __init__(self, 
                emb_matrix,
                embedding_dim,
                n_filters_per_size,
                filter_sizes, # limited to len=3 (e.g.for 3,4,5-grams)
                n_classes,  
                device,
                dropout=0.5,
                padding_value=0):
        ...
        self.fc = nn.Linear(in_features=(n_filters_per_size * len(filter_sizes)), out_features=n_classes)
    ...
    def forward(self, x):
        ...
        fw_output = self.fc(feature_map)
        fw_output = self.softmax(fw_output)
        return fw_output
    

# 3 ) KBBERT
class KB_LongBERT(nn.Module):
    def __init__(self,
                 bert_model_path,
                 n_classes,
                 tokenizer,
                 ...
                 dropout=0.1,
                 chunksize=512):
        super(KB_LongBERT, self).__init__()
        self.n_classes = n_classes
        ...
        # to account for any new tokens added to the tokenizer
        self.kb_bert_layer.resize_token_embeddings(len(self.tokenizer)) 
    ...
    def forward(self, input_ids, mask):
        ...
        if input_ids.shape[1] <= self.chunksize:
            ...
            # cls_token_out = bert_out["last_hidden_state"][:, 0, :]  
            
            # Use pooler output, which is similar to raw CLS has additional linear and tanh function on top
            cls_token_out = bert_out["pooler_output"]   
            # (batchsize, sequence length, embedding_dim) -> take the embedding of the CLS (== 0th) token of the sequence for each batch
            cls_token_out = self.dropout(cls_token_out)
            fw_output = self.classifier(cls_token_out)
            fw_output = self.softmax(fw_output)
            return fw_output
        ...
        # collect logits for each text -> feed it through softmax as a batch tensor again
        logits = torch.zeros(input_ids.shape[0], self.n_classes).to(self.device) 
        ...
        for i, input_ids_one_text in enumerate(input_ids):
            ...
            # merge k logits into a single logits with maxpooling
                ...
                cls_token_out = self.dropout(cls_token_out)
                fw_output = self.classifier(cls_token_out)
                ## merge k segments into one maxpooled vector 
                logits[i] = torch.max(fw_output, dim=0).values # dim to collapse = n_chunks
                
        fw_output = self.softmax(logits)
        return fw_output


# 4 ) HiSAN
# Taken from the Fresco library 
# https://github.com/DOE-NCI-MOSSAIC/FrESCO/blob/main/fresco/models/mthisan.py
# adapted to fit the pipeline, i.e. the embedding matrix is initialized and processed differently, to match the (MT)CNN models
...
class HiSAN(nn.Module):
    ...
    def __init__(self,
        ...
        # Adapted to be only for single task
    ...
    def forward(self, docs, return_embeds=False):
        ...
        logits = self.fc(doc_embeds)                                      # batch x num_classes
        output_probs = self.softmax(logits)
        if return_embeds:
            return output_probs, doc_embeds
        return output_probs
