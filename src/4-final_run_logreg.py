...
from functions_transform_data import TrainVocabulary, TF_IDF_Dataset
...
for label_col in ["morphology", "behavior", "histology", "site"]:
    for suffix in ["_False", "",]:
        ...
        logreg_dir = os.path.join(experiments_dir, config.model_type)
        create_folder_if_not_exists(logreg_dir)

        # add final cross validation loop:
        cv_dir = os.path.join(logreg_dir, f"{config.model_type}_cv_{label_col[:3]}")
        ...
        for i, (train_ind, valid_ind) in enumerate(kfolds.folds[1:], start=1):
            ...
            dataset_train = TF_IDF_Dataset(train_valid_data[text_col][train_ind],
                                    ...
                                    vectorizer=None,
                                    replace_oov=True)
            dataset_val = TF_IDF_Dataset(train_valid_data[text_col][valid_ind], 
                                    train_valid_data[label_col][valid_ind], 
                                    voc, 
                                    label_map=label_mapper.label_map,
                                    vectorizer_name=dataset_train.vectorizer_name,    # from train
                                    vectorizer=dataset_train.vectorizer,              # from train
                                    replace_oov=True)
            
            dataset_test = TF_IDF_Dataset(test_data[text_col],
                                            test_data[label_col],
                                            vocab=voc,
                                            label_map=label_mapper.label_map,
                                            vectorizer_name=dataset_train.vectorizer_name,
                                            vectorizer=dataset_train.vectorizer,
                                            replace_oov=True)
            ...
            model = LogisticRegression(n_in_features=dataset_train.n_features,
                                    n_classes=n_classes,
            ...
            test_metrics, label_list = test_saved_model(model=model, 
                                                        checkpoint_path=os.path.join(one_experiment_output_folder, 
                                                                                    f"best_{config.experiment_name}.pt"),
